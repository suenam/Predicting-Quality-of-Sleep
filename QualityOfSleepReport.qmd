...
Our decision tree model will use the predictors of *Gender, Age, Occupation, Sleep Duration, Physical Activity Level, Stress Level, BMI Category, Heart Rate, Daily Steps,* and *Disorders* to predict the *Quality of Sleep*.
...
From the model, we achieved a Mean Test Error Rate (MSE) of 0.2054233, meaning that our models predictions have an average squared deviation of 0.2054233 from the true values.

To avoid overfitting the data, we pruned the tree.

### Pruned Tree

Lets perform cross validation on our model to determine the optimal size to prune our tree to.
...
From the plot, it appears that the optimal size may be at 3 (the 'elbow' of the plot) as the line seems to start to flatten after around this point, indicating the size beyond this point doesn't improve the model by much.
...
From the model, we achieved a Mean Test Error Rate (MSE) of 0.378096, meaning that our models predictions have an average squared deviation of 0.378096 from the true values. This is a higher MSE than the unpruned tree, indicating that we lost some accuracy in the process of pruning the tree.

To further improve our model, we created a random forest model.

### Random Forest

Creating a random forest model with 500 trees...
...
The importance as well as the plotted random forest
...
From the model, we achieved a Mean Test Error Rate (MSE) of 0.1228698, meaning that our models predictions have an average squared deviation of 0.1228698 from the true values. This is better than the original decision tree and pruned decision tree.
